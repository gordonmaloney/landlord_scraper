...
search_term = input('Search for respondent: ')
...
def search_page(full_url, page):
    ...
    for match in soup.findAll('tr', {'class': ['odd', 'even']}):
        if search_term.lower() in match.find('td', class_='views-field-field-respondent-other').text.lower():
            ...
            print('Link to decision: ' + match.findAll('a', href=True)[1]['href'])
